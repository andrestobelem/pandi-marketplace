...
`notify <kind>` y `notify-text <mensaje> [kind]` (usados por los hooks), y
...
fuente de texto (`src/font.ts`, 5x7), la lógica de opciones de `ask`/`confirm`
(`src/ask.ts`) y el matcher de comandos riesgosos (`src/risky-command.ts`) son módulos
puros separados, todos con tests unitarios (vitest) en `tests/`. Se verificó en vivo
contra un Launchpad X real: pad individual, las animaciones/patterns, texto
scrolleando, `ask`/`confirm`, `wait-for-press`, y los dos hooks de notificación.
